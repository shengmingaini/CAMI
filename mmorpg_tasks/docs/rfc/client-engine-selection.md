...
| §6.1 TASK-035 | Godot 渲染管线 + 双档 + LOD / 合批 / 距离剔除 | 改为 **2D 渲染管线**：`TileMapLayer` + `Sprite2D`/`AnimatedSprite2D` + Y-sort 遮挡 + 2D 粒子 + `CanvasLayer` UI；**性能预算改 2D 口径**（见 §9.3） |
| §6.1 TASK-036 | 资源管线 + 三档预算（贴图/面数/DrawCall/音频）+ 3D Chunk Streaming | 改为 **2D 资源管线**：图集（atlas）+ `.import` 配置 + 音频流式；**分块加载改 `TileMap` 区块**（替代 3D Chunk Streaming） |
| §7.2-2 美术源格式 | 美术源用 glTF / FBX | 改为 **PNG（带 alpha）/ SVG（UI 图标）/ 图集 `.tres`**；glTF/FBX 仅 3D 阶段启用 |
| §6.1 新增客户端任务数 | 15–20 个 | **12–16 个**（见 §9.4） |

### 9.3 性能预算口径改写（2D）

原 3D 口径（DrawCall / 三角面数 / LOD 级数 / 阴影质量）在 2D 下不适用，改为：
...
→ **新增客户端任务：12–16 个**（原 15–20）。省下的大头是 3D 资产管线，不是代码量。
...
1. **2D 画风/视角**：俯视四方向（top-down）vs 等距 2.5D（isometric）——
   影响 Tile 尺寸、Y-sort 策略、角色精灵方向数（4 向 / 8 向）、相机跟随方式。
2. **Godot 编辑器获取**：同 §7.2-5（一次性联网，约 72MB ZIP，之后离线可用）。
3. **TASK-034/035/036 重写时机**：见 §9.8。
4. **是否补充「2.5D 过渡档」**：等距视角 + 2D 精灵可近似 3D 观感，
   是 2D 到 3D 之间的低成本中间态，建议作为演进路径候选而非当前范围。
