/// TASK-034 · NetLink 实现（winsock2 客户端 + 4 字节大端长度前缀帧）。
...
void NetLink::PutBe32(std::uint8_t* p, std::uint32_t v) noexcept {
...
std::uint32_t NetLink::GetBe32(const std::uint8_t* p) noexcept {
...
core::Result<void> NetLink::Connect(std::string_view addr,
    ...
    if (colon == std::string_view::npos) {
        return core::Result<void>::Fail(core::Error(
            core::ErrorCode::INVALID_ARGUMENT, "gateway addr 缺少端口", core::domain::kNet));
    }
    ...
    if (port <= 0 || port > 65535) {
        return core::Result<void>::Fail(core::Error(
            core::ErrorCode::INVALID_ARGUMENT, "端口非法", core::domain::kNet));
    }
    ...
    if (getaddrinfo(host.c_str(), port_str.c_str(), &hints, &res) != 0 || res == nullptr) {
        return core::Result<void>::Fail(core::Error(
            core::ErrorCode::NOT_FOUND, "无法解析网关地址", core::domain::kNet));
    }
    ...
    return core::Result<void>::Fail(core::Error(
        core::ErrorCode::TIMEOUT, "连接网关超时", core::domain::kNet));
}

core::Result<void> NetLink::Send(std::string_view payload) {
    if (!IsOpen()) {
        return core::Result<void>::Fail(core::Error(
            core::ErrorCode::UNAUTHORIZED, "连接未建立", core::domain::kNet));
    }
    if (payload.size() > (1u << 20)) {
        return core::Result<void>::Fail(core::Error(
            core::ErrorCode::INVALID_ARGUMENT, "payload 超过 1MiB", core::domain::kNet));
    }
...
core::Result<std::vector<std::uint8_t>> NetLink::Recv(std::uint32_t recv_timeout_ms) {
    ...
    if (recv_timeout_ms > 0) {
        ...
        if (sr == 0) {
            return core::Result<std::vector<std::uint8_t>>::Fail(core::Error(
                core::ErrorCode::TIMEOUT, "接收超时", core::domain::kNet));
        } else if (sr < 0) {
            return core::Result<std::vector<std::uint8_t>>::Fail(core::Error(
                core::ErrorCode::INTERNAL_ERROR, "select 失败", core::domain::kNet));
        }
    }
