...
/// 另支持 --matrix（TASK-025 钩子）：跑到多尺寸矩阵并写 bench/combat_matrix.json。
///
...
double RunPhase(Harness& h, std::size_t entities, double ratio, std::size_t rounds,
    ...
    h.combat.Update(SceneContext(kScene, SceneType::World, kOwner, base, 0, h.mgr, h.bus, h.sched, h.arena));
    for (std::size_t i = 0; i < in_combat; ++i) {
        EntityId enemy = ids[(i + 1) % entities];
        h.combat.EnterCombat(ids[i], enemy, 1);
    }
    // 热身几轮
    for (std::size_t r = 0; r < 50; ++r) {
        auto now = base + std::chrono::milliseconds(static_cast<std::int64_t>(r) * 50);
        h.combat.Update(SceneContext(kScene, SceneType::World, kOwner, now, r, h.mgr, h.bus, h.sched, h.arena));
    }
    // 计时轮
    ...
    for (std::size_t r = 0; r < rounds; ++r) {
        ...
        h.combat.Update(SceneContext(kScene, SceneType::World, kOwner, now, 1000 + r, h.mgr, h.bus, h.sched, h.arena));
    }
...
int main(int argc, char** argv) {
    ...
    if (matrix) {
        // TASK-025 钩子：多种子 / 多尺寸矩阵，输出 combat_matrix.json
        std::vector<std::size_t> sizes = {500, 1000, 2000};
        std::ofstream fjson("bench/combat_matrix.json");
        fjson << "{\"scenarios\":[";
        bool first = true;
        for (std::size_t n : sizes) {
            std::size_t allocs = 0;
            double us = RunPhase(h, n, ratio, ticks, allocs);
            double phase = (us / static_cast<double>(ticks)) * 1000.0 / static_cast<double>(n);
            if (!first) fjson << ",";
            first = false;
            fjson << "{\"entities\":" << n << ",\"combat_ratio\":" << ratio
                  << ",\"combat_phase_us_at_1k\":" << phase
                  << ",\"alloc_per_combat_tick\":" << (allocs / ticks) << "}";
        }
        fjson << "]}";
        fjson.close();
        ErrorFmt("bench: matrix written to bench/combat_matrix.json\n");
    }
    ...
    h.combat.Update(SceneContext(kScene, SceneType::World, kOwner, core::MonotonicClock::Point(),
                                 0, h.mgr, h.bus, h.sched, h.arena));
    h.combat.EnterCombat(caster, target, 1);
    // 热身
    for (int i = 0; i < 100; ++i) {
